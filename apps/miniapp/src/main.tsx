...
type Cards = { materials: number; chapter: number; cards: { key: string; level: number }[]; battle?: { player_hp: number; enemy_hp: number; turn: number; choices: string[] }; tower?: { floor: number; points: number; ended: boolean; strength: number; is_elite_floor: boolean; max_floor: number } };
type Checkin = { played_today: boolean; claimed_today: boolean; streak: number; can_claim: boolean; collection_awarded?: number };
type Leaderboard = { period: string; scope: string; metric: string; entries: { rank: number; name: string; level?: number; xp?: number; points?: number; is_me: boolean }[] };
...
type Faq = { locale: string; entries: { question: string; answer: string }[] };
type NotificationPreferences = { crop_mature: boolean; idle_full: boolean; daily_checkin: boolean; activities: boolean; seasons: boolean };
...
function App() {
  ...
  const loadLeaderboard = async (period = 'all_time', scope = 'global') => { try { setLeaderboard(await request(`/api/leaderboards/farm?period=${period}&scope=${scope}`)); } catch (error) { notify(error instanceof Error ? error.message : 'Leaderboard unavailable'); } };
  const loadProfile = async () => { try { const [profileState, visitorState, faqState, notificationState] = await Promise.all([request('/api/profile'), request('/api/profile/visitors'), request(`/api/support/faq?locale=${locale}`), request('/api/profile/notification-preferences')]); setProfile(profileState); setVisitors(visitorState); setFaq(faqState); setNotificationPreferences(notificationState); } catch (error) { notify(error instanceof Error ? error.message : 'Profile unavailable'); } };
  ...
  const requestReset = async (game: string) => { try { const response = await fetch('/api/support/reset-request', { method: 'POST', headers: { 'Content-Type': 'application/json', ...headers() }, body: JSON.stringify({ game }) }); if (!response.ok) { const data = await response.json(); throw new Error(data.detail || 'Reset request unavailable'); } notify(`${game} reset request sent to support.`); } catch (error) { notify(error instanceof Error ? error.message : 'Reset request unavailable'); } };
  const updateNotificationPreferences = async (next: NotificationPreferences) => { try { const response = await fetch('/api/profile/notification-preferences', { method: 'PUT', headers: { 'Content-Type': 'application/json', ...headers() }, body: JSON.stringify(next) }); if (!response.ok) throw new Error('Notification preference unavailable'); setNotificationPreferences(await response.json()); } catch (error) { notify(error instanceof Error ? error.message : 'Notification preference unavailable'); } };
  ...
  const action = game === 'farm'
    ...
    : game === 'pets'
      ...
      : { text: t.chapter, run: () => void perform('/api/cards/battle/strike', 'Card turn resolved.') };
  ...
  return <main className="app-shell"><header><div><p className="eyebrow">RETROHUB TEST</p><h1>{gameMeta[game].title}</h1></div><button className="avatar" aria-label="Telegram profile" onClick={() => changeLocale(locale === 'en' ? 'zh' : locale === 'zh' ? 'ru' : 'en')}>{locale.toUpperCase()}</button></header>
    ...
    {game === 'cards' && cards && <section className="detail-panel"><b>{t.forge}</b><p>{cards.cards.length ? cards.cards.map((card) => `${card.key.replace('_', ' ')} Lv.${card.level}`).join(' · ') : t.noCards}</p><button onClick={() => void perform('/api/cards/craft/clockwork_fox', 'Clockwork Fox forged!')}>{t.craft}</button>{cards.battle && <><p>Chapter battle · You {cards.battle.player_hp} HP · Enemy {cards.battle.enemy_hp} HP · Turn {cards.battle.turn}</p><button onClick={() => void perform('/api/cards/battle/strike', 'Strike resolved.')}>Strike</button><button onClick={() => void perform('/api/cards/battle/guard', 'Guard resolved.')}>Guard</button><button onClick={() => void perform('/api/cards/battle/focus', 'Focus resolved.')}>Focus</button></>}{cards.tower && <p>Weekly tower: floor {cards.tower.floor}/{cards.tower.max_floor} · {cards.tower.points} points · strength {cards.tower.strength}{cards.tower.is_elite_floor ? ' · Elite floor' : ''}</p>}<button disabled={cards.tower?.ended} onClick={() => void perform('/api/cards/tower/challenge', 'Tower challenge resolved.')}>{cards.tower?.ended ? 'Tower run ended' : 'Challenge tower'}</button></section>}
    {leaderboard && <section className="detail-panel"><b>{t.leaderboard} · {leaderboard.scope} · {leaderboard.period.replace('_', ' ')}</b><p>{leaderboard.entries.length ? leaderboard.entries.slice(0, 5).map((entry) => `#${entry.rank} ${entry.name} ${entry.points === undefined ? `Lv.${entry.level}` : `${entry.points} pts`}${entry.is_me ? ' (You)' : ''}`).join(' · ') : t.noRanks}</p><button onClick={() => void loadLeaderboard('all_time', 'global')}>Global</button><button onClick={() => void loadLeaderboard('all_time', 'friends')}>Friends</button><button onClick={() => void loadLeaderboard('week', leaderboard.scope)}>Weekly orders</button><button onClick={() => void loadLeaderboard('month', leaderboard.scope)}>Monthly orders</button></section>}
    ...
    {faq && <section className="detail-panel"><b>FAQ</b>{faq.entries.map((entry) => <p key={entry.question}><strong>{entry.question}</strong><br />{entry.answer}</p>)}</section>}
    ...
    {notificationPreferences && <section className="detail-panel"><b>Notifications</b><p>Important reminders are on by default; activities and seasons are optional.</p><button onClick={() => void updateNotificationPreferences({ ...notificationPreferences, crop_mature: !notificationPreferences.crop_mature })}>Crop mature: {notificationPreferences.crop_mature ? 'On' : 'Off'}</button><button onClick={() => void updateNotificationPreferences({ ...notificationPreferences, idle_full: !notificationPreferences.idle_full })}>Idle full: {notificationPreferences.idle_full ? 'On' : 'Off'}</button><button onClick={() => void updateNotificationPreferences({ ...notificationPreferences, daily_checkin: !notificationPreferences.daily_checkin })}>Daily check-in: {notificationPreferences.daily_checkin ? 'On' : 'Off'}</button><button onClick={() => void updateNotificationPreferences({ ...notificationPreferences, activities: !notificationPreferences.activities })}>Activities: {notificationPreferences.activities ? 'On' : 'Off'}</button><button onClick={() => void updateNotificationPreferences({ ...notificationPreferences, seasons: !notificationPreferences.seasons })}>Seasons: {notificationPreferences.seasons ? 'On' : 'Off'}</button></section>}
    {profile && <section className="detail-panel"><b>Request a game reset</b><p>Data resets are reviewed by support. No game data is deleted automatically.</p><button onClick={() => void requestReset('farm')}>Request farm reset</button><button onClick={() => void requestReset('pets')}>Request pet reset</button><button onClick={() => void requestReset('cards')}>Request card reset</button></section>}
