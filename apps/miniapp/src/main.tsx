...
function App() {
  ...
  const loadProfile = async () => { try { const [profileState, visitorState, faqState] = await Promise.all([request('/api/profile'), request('/api/profile/visitors'), request(`/api/support/faq?locale=${locale}`)]); setProfile(profileState); setVisitors(visitorState); setFaq(faqState); } catch (error) { notify(error instanceof Error ? error.message : 'Profile unavailable'); } };
